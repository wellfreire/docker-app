...
NGINX_ECHO_MODULE_URL="https://github.com/openresty/echo-nginx-module/archive/v0.58.tar.gz"
...
apt-get update
apt-get install -y gcc g++ make libc6-dev libpcre++-dev libssl-dev libxslt-dev libgd2-xpm-dev libgeoip-dev wget

download_and_extract "${NGINX_DOWNLOAD_URL}" "${NGINX_SETUP_DIR}/nginx"
download_and_extract "${NGINX_ECHO_MODULE_URL}" "${NGINX_SETUP_DIR}/nginx-echo-module"
...
./configure --prefix=/usr/share/nginx --conf-path=/etc/nginx/nginx.conf --sbin-path=/usr/sbin \
  ...
  --add-module=${NGINX_SETUP_DIR}/nginx-echo-module
make && make install
